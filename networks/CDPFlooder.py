...
def generatePacket():
    ...
    return etherframe / llcFrame / snapFrame / cdpHeader / cdpGeneric / cdpGeneric2

def flooder(num, packets):
    Logger.info(f'Starting task: {num}, packets num: {len(packets)}')
    ...
    packetsGen = [generatePacket() for _ in range(512)]
    if len(packets) == 0:
...
'''.format(VERSION))
    ...
    Logger.info(f"Will use {config['processors']} processors.")
...
def main(argv):
    ...
    packetsLists = [[] for _ in range(config['processors'])]
